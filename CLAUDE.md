...
## Overview
Shared gRPC/protobuf definitions (Go + Python) for Shipthis internal services, covering auth token validation and inter-service RPC contracts.
...
5. **Verify before done** — invoke `superpowers:verification-before-completion` before declaring the task complete. Run `go vet ./...` and confirm tests pass.
